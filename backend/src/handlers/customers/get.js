...
async function handler(event) {
  ...
  try {
    const rs = await query(`SELECT * FROM customers WHERE id = $1 AND account_id = $2 AND deleted_at IS NULL LIMIT 1`, [customerId, ctx.accountId]);
    const row = rs.rows?.[0] || null;
    if (!row) return fail(404, "NOT_FOUND", "Customer not found");
    const outstanding = await getCustomerOutstandingInfo(query, ctx.accountId, customerId);
    return ok({ customer: mapCustomerRow(row), outstanding });
  } catch {
