...
async function handler(event) {
  ...
  const raw = body.globalDiscountPercent ?? body.global_discount_percent;
  const discountPercent = Number(raw);
  if (!Number.isFinite(discountPercent) || discountPercent < 0 || discountPercent > 100) {
    return fail(400, "VALIDATION_ERROR", "globalDiscountPercent must be between 0 and 100.");
  }
  ...
  try {
    const data = await withTransaction(async (q) => {
      ...
      return { invoiceId, globalDiscountPercent: discountPercent, totals: validated.totals, warnings: validated.warnings || [] };
    });
    if (data?.err) return data.err;
    return ok(data, { message: "Global discount applied to all items." });
  } catch (e) {
