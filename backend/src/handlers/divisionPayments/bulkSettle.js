  ...
  try {
    const data = await withTransaction(async (q) => {
      ...
      for (const invoiceId of invoiceIds) {
        const inv = await q(
          `SELECT id, invoice_number, division_id, status, payment_status, balance_due
        ...
        if (!invoice.division_id) {
          skipped.push({ invoiceId, invoiceNumber: invoice.invoice_number, reason: "NO_DIVISION" });
          continue;
        }
    ...
    return ok(data, { message: data.completedCount ? "Bulk division payments recorded." : "No eligible invoices to settle." });
  } catch (e) {
