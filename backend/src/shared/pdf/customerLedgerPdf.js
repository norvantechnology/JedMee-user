...
async function buildCustomerLedgerPdfAttachment(ledgerDoc) {
  ...
  const buffer = await pdfToBuffer((pdf) => {
    ...
    pdf.moveDown(0.5);
    pdf.fontSize(11).fillColor("#4c2480").text("Customer Ledger", { underline: true });
    pdf.moveDown(0.3);
    pdf.fontSize(10).fillColor("#1a0c30");
    pdf.text(`Customer: ${cust.name || ""}`);
    if (cust.code) pdf.text(`Code: ${cust.code}`);
    if (cust.full_address) pdf.text(`Address: ${cust.full_address}`);
    pdf.moveDown(0.4);
    pdf.fontSize(9).fillColor("#4c2480").text(
      `Net balance: ${inr(summary.netBalance)}  |  Due: ${inr(summary.balanceDue)}  |  Advance: ${inr(summary.advanceAmount)}`
    );
    pdf.moveDown(0.6);
    ...
    for (const e of entries) {
      ...
      pdf.text(String(e.type_label || e.type || "").slice(0, 28), left + c1, y, { width: c2 });
  ...
  const filename = `Customer-Ledger-${safeFilePart(cust.code || cust.name)}.pdf`;
  return { buffer, filename };
}

module.exports = { buildCustomerLedgerPdfAttachment };
