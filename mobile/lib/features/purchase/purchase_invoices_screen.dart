...
class _PurchaseInvoicesScreenState extends ConsumerState<PurchaseInvoicesScreen> {
  ...
  List<({String id, String label})> _suppliers = [];
  ...
  Future<void> _bulkEmail(BuildContext context) async {
    ...
    final ok = await showConfirmDialog(
      ...
      message: 'Emails will be sent to the respective suppliers.',
      destructive: false,
    );
    if (ok != true || !context.mounted) return;
    final ids = _selectedIds.toList();
    final resp = await ref.read(purchaseRepositoryProvider).sendPurchaseInvoicesByEmail({'ids': ids});
    if (!mounted) return;
    if (resp.ok) {
  ...
  Future<void> _bulkCompletePayment(BuildContext context) async {
    ...
    await withSavingOverlay(context, () async {
      final resp = await ref.read(paymentRepositoryProvider).bulkSettleVendorPayments({
        'invoiceIds': ids,
        'paymentDate': result['paymentDate'],
        'paymentMode': result['paymentMode'],
        'notes': 'Bulk payment settled from purchase invoices',
      });
      ...
      pageBuilder: (ctx, animation, secondaryAnimation) => StatefulBuilder(
        builder: (ctx, setDialogState) => Center(child: AlertDialog(
          ...
          content: Column(
            ...
            children: [
              Text('Payment date', style: AppTypography.inputLabel),
                ...
                child: DropdownButtonHideUnderline(
                  child: DropdownButton<String>(
                    ...
                    items: const ['CASH', 'UPI', 'CARD', 'CHEQUE', 'NEFT', 'OTHER']
                        ...
                        .toList(),
                    onChanged: (v) => setDialogState(() => paymentMode = v ?? 'NEFT'),
                  ),
                ),
              ),
            ],
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      if (mounted) {
        ref.read(ongoingPurchaseBillsProvider.notifier).setActive(null);
      }
    });
  }
  ...
  Future<void> _loadSuppliers() async {
    ...
    List<dynamic> items = [];
    if (raw is Map) {
      items = (raw['data'] ?? raw['vendors'] ?? raw['items'] ?? []) as List;
    } else if (raw is List) {
      items = raw;
    }
    final suppliers = items
        ...
        .where((v) => v.id.isNotEmpty && v.label.isNotEmpty)
        .toList();
  ...
  Future<void> _openEditor({String? id, Map<String, dynamic>? initialBatch}) async {
    ...
    final changed = await context.push<bool>(
      id == null ? '/purchase-invoices/new' : '/purchase-invoices/edit/$id',
      extra: initialBatch,
    );
  ...
  /// Scan a barcode from the listing screen — always opens a NEW purchase
  /// invoice pre-populated with the scanned batch. Never opens an existing draft.
  Future<void> _scanAndAddToActive() async {
    ...
    // Always open a new purchase invoice from the listing screen scan button.
  ...
  Future<void> _showDetail(Map<String, dynamic> row, Future<void> Function() refresh) async {
    ...
    final detail = await withDetailLoading(context, () async {
      final resp = await ref.read(purchaseRepositoryProvider).getPurchaseInvoice(id);
      return parsePurchaseInvoiceDetail(resp);
    });
    if (!mounted) return;
    ...
    showTxnInvoiceDetail(
      ...
      canEdit: status == 'DRAFT' && can(auth, 'PURCHASE_INVOICES', 'UPDATE'),
      canConfirm: status == 'DRAFT' && can(auth, 'PURCHASE_INVOICES', 'CONFIRM'),
      canCancel: (status == 'DRAFT' || status == 'CONFIRMED') && can(auth, 'PURCHASE_INVOICES', 'CANCEL'),
      canDelete: (status == 'DRAFT' || status == 'CANCELLED') && can(auth, 'PURCHASE_INVOICES', 'DELETE'),
      canRecordPayment: hasBalance && can(auth, 'VENDOR_PAYMENTS', 'ADD'),
      canSendEmail: can(auth, 'PURCHASE_INVOICES', 'VIEW'),
      ...
      onConfirm: () async {
        ...
        final vendorId =
            (header['vendor_id'] ?? header['vendorId'] ?? '').toString();
        final divisionId =
            (header['division_id'] ?? header['divisionId'] ?? '').toString();
        final prefs = await showInvoiceConfirmPaymentDialog(
          context,
          isSales: false,
          totalAmount: total > 0 ? total : parseBalanceDue(header),
          initial: defaultPurchasePaymentPrefs(),
          title: 'Confirm bill?',
          subtitle: 'Stock will come in.',
        );
        ...
        final err = await confirmInvoiceWithPayment(
          ...
          divisionId: divisionId.isNotEmpty ? divisionId : null,
        );
        if (!mounted) return;
        if (err == null) {
          invalidateBillingMasterCache();
      ...
      onDelete: () async {
        final resp =
            await ref.read(purchaseRepositoryProvider).deletePurchaseInvoice(id);
        if (!mounted) return;
        if (resp.ok) {
      ...
      canPrint: status == 'CONFIRMED' && can(auth, 'PURCHASE_INVOICES', 'VIEW'),
      onShare: status == 'CONFIRMED' && can(auth, 'PURCHASE_INVOICES', 'VIEW')
          ? () async {
              final pdf = PdfService(
                ref.read(salesRepositoryProvider),
                ref.read(purchaseRepositoryProvider),
              );
              await pdf.sharePurchaseInvoicePdf(
                id,
                invoiceNo: (header['invoice_number'] ?? header['invoiceNumber'])?.toString(),
              );
            }
          : null,
      onDownloadPdf: status == 'CONFIRMED' && can(auth, 'PURCHASE_INVOICES', 'VIEW')
          ? () async {
              final pdf = PdfService(
                ref.read(salesRepositoryProvider),
                ref.read(purchaseRepositoryProvider),
          ...
          : null,
      onPrint: () async {
        final err = await InvoicePrintService(
          ref.read(salesRepositoryProvider),
          ref.read(purchaseRepositoryProvider),
      ...
      onSendEmail: () async {
        final resp = await ref
            ...
            .sendPurchaseInvoicesByEmail({'ids': [id]});
        if (!mounted) return;
        if (resp.ok) {
  ...
  Future<void> _showPaymentSheet(
    ...
    final result = await showAppBottomSheet<Map<String, dynamic>>(
      ...
      builder: (_) => _PurchasePaymentSheet(
        ...
        vendorId: (row['vendor_id'] ?? row['vendorId'] ?? '').toString(),
        divisionId: (row['division_id'] ?? row['divisionId'] ?? '').toString(),
      ),
    );
    if (result == null || !mounted) return;

    // Determine payment type (vendor or division)
    final vendorId = result['vendorId']?.toString() ?? '';
    final divisionId = result['divisionId']?.toString() ?? '';

    ApiResponse resp;
    if (vendorId.isNotEmpty) {
      resp = await ref.read(paymentRepositoryProvider).createVendorPayment(result);
    } else if (divisionId.isNotEmpty) {
      resp = await ref.read(paymentRepositoryProvider).createDivisionPayment(result);
    } else {
      showAppSnack(
        context,
        message: 'No vendor or division linked to this invoice',
        type: AppSnackType.error,
      );
      return;
    }
  ...
  Widget build(BuildContext context) {
    ...
    final canAdd = can(auth, 'PURCHASE_INVOICES', 'ADD');
    final canConfirm = can(auth, 'PURCHASE_INVOICES', 'CONFIRM');
    final canCancel = can(auth, 'PURCHASE_INVOICES', 'CANCEL');
    ...
    return PermissionGate(
      ...
      title: 'Purchases',
      child: AppShell(
        title: _selectionMode ? '${_selectedIds.length} selected' : 'Purchases',
        bottomBar: _selectionMode
          ...
          leadingActions: [
            BottomAction(
              ...
              tooltip: 'Scan barcode into active purchase',
              onTap: canAdd ? _scanAndAddToActive : null,
              enabled: canAdd,
            ),
            BottomAction(
              icon: AppIcons.importFile,
              tooltip: 'Import purchases (CSV)',
  ...
  Widget _buildList(bool canAdd) {
    return TxnListPage(
      ...
      load: (search, status) async {
        ...
        final resp = await ref.read(purchaseRepositoryProvider).listPurchaseInvoices({
...
class _PurchasePaymentSheetState extends State<_PurchasePaymentSheet> {
  ...
  void _submit() {
    ...
    if (widget.vendorId.isNotEmpty) {
      payload['vendorId'] = widget.vendorId;
    } else if (widget.divisionId.isNotEmpty) {
      payload['divisionId'] = widget.divisionId;
    }
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              ...
              const Text('Record Payment', style: AppTypography.cardTitle),
              ...
              AppDropdownField<String>(
                ...
                items: _kPaymentModes
                    .map((m) => DropdownMenuItem(value: m, child: Text(m)))
                    .toList(),
                onChanged: (v) => setState(() => _paymentMode = v ?? 'NEFT'),
              ),
              const SizedBox(height: AppSpacing.sm),

              // Reference
              const _PayLabel('Reference number (optional)'),
              const SizedBox(height: 5),
              TextFormField(
                controller: _refCtrl,
                style: AppTypography.body,
              ...
              // Notes
              const _PayLabel('Notes (optional)'),
              const SizedBox(height: 5),
              TextFormField(
